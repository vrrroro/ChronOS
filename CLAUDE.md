...
- **Never bind a `ListView`'s `model` directly to a `Q_INVOKABLE` call result that changes every tick** (fixed 2026-08-25 in `ProcessTicker.qml` — see `CHANGELOG.md`). `bridge.processTickerAt(tick)` returns a fresh `QVariantList` object every call; QML can't diff that against the previous one, so `ListView` tears down and recreates every delegate every tick, which silently kills any `Behavior` animation running on them — a freshly created item's first binding evaluation never triggers a `Behavior`, it just snaps to the target. The fix pattern: bind `model` to something with **stable identity across ticks** (a `Q_PROPERTY` count, unchanging for the run) and have each delegate pull its own live data via a per-index `Q_INVOKABLE` (`processTickerRowAt(tick, index)`), so the same delegate instances persist and their `Behavior`s actually get to run. `ReadyQueueTable`/`AgingIndicator` still bind straight to a per-tick call (`readyQueueAt`/`agingListAt`) and technically churn the same way — left as-is since neither has a `Behavior` animation that would make it visible, but don't copy that pattern into anything that does.
