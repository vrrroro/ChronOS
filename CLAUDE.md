...
- **Per-process identity is neon hue AND glyph, two channels, indexed `pid % 8`** (`EDRD.md` §2.3). This reverted the green-brightness-step scheme of 2026-08-24 after one day: eight steps of one hue are steps a viewer has to *compare* rather than *recognize*, and the middle four were not separable at ticker size. The glyph channel is **kept alongside** the hue, not replaced by it — that is what carries the encoding under greyscale, a projector, or deuteranopia, and it is the reason reintroducing hue was safe. Do not drop either channel. Use `Theme.procShade(pid)` / `Theme.procGlyph(pid)`; never index the arrays by hand.
...
- Do not remove the glyph channel from per-process identity now that hue is back. Both channels are specified (`EDRD.md` §2.3); the glyph is the accessibility fallback that made reintroducing hue defensible.
