...
| GUI | Native Qt 6/QML app (`app/`, target `chronos_app`) on the **Terminal CLI** visual system, revised 2026-08-24b: green is chrome only, five ANSI hues each own exactly one app-wide role (`EDRD.md` §2.5), per-process identity is hue+glyph (`§2.3`), bundled JetBrains Mono + Instrument Serif, liquid `FluidBar` process ticker, hover `ProcessHud`, slowed default playback (0.6 ticks/sec). Builds clean, launches with **zero QML warnings on stderr**. |
...
- [x] **2026-08-25:** full doc sync (`PRD.md` §5.1/§5.3/§6.3, `ARCHITECTURE.md` §3.4/§4.3/§5.1/§5.3, `README.md`, this file) to match the 2026-08-24b revision; build, 40-run engine smoke test, and GUI stderr check all re-verified clean before committing.
